...
def process_zarr(path, output_zarr, type):
    ...
    if click.confirm("Perform bounding box crop?", default=False if type == 'raw' else True, show_default=True):
        subprocess.run(["bs", "utils", "bbox", "-i", os.path.join(path, in_ds_name), "-o", os.path.join(output_zarr, out_ds_name)])
...
def process_non_zarr(path, output_zarr, type):
    ...
    voxel_size = tuple(click.prompt("Enter voxel size (space separated integers)", default="1 1 1", type=str, show_default=True).split())
    voxel_offset = tuple(click.prompt("Enter voxel offset (space separated integers)", default="0 0 0", type=str, show_default=True).split())
    axis_names = tuple(click.prompt("Enter axis names (space separated strings)", default="z y x", type=str, show_default=True).split())
    units = tuple(click.prompt("Enter units (space separated strings)", default="nm nm nm", type=str, show_default=True).split())
    crop = click.confirm("Perform bounding box crop?", default=False if type == 'raw' else True, show_default=True)

    args = ["bs","utils","convert","-i",path,"-o",out_array,"-d",dtype,"-vs", *voxel_size,"-vo", *voxel_offset,"-ax", *axis_names,"-u", *units]
    if crop:
        args.append("-c")
...
def process_dataset(path, output_zarr, type):
    ...
    if click.confirm("Generate scale pyramid?", default=False):
        ...
        scales = tuple(click.prompt("Enter scales for each axis (space separated integers)", default="1 2 2", type=str, show_default=True).split())
        chunk_shape = tuple(click.prompt("Enter chunk shape (space separated integers)", default="8 256 256", type=str, show_default=True).split())
        mode = click.prompt("Enter mode", type=click.Choice(['up', 'down']), show_default=True, default="down")

        subprocess.run(["bs", "utils", "scale-pyramid", "-f", in_file, "-ds", in_ds_name, "-s", *scales, "-c", *chunk_shape, "-m", mode])
    ...
    if click.confirm(f"Make {type} mask?", default=False):
        ...
        subprocess.run(["bs", "utils", "mask", "-i", out_ds_name, "-o", mask_ds_name, "-m", type])
    else:
        mask_ds_name = None
...
def prepare_volume(base_dir, volume_index):
    output_zarr = click.prompt("Enter path to output zarr container", default=os.path.join(base_dir, f"volume_{volume_index + 1}.zarr"), type=click.Path())
...
def make_volumes(base_dir, num_volumes):
    ...
    logger.info(f'Processing volumes in {base_dir}, number of volumes: {num_volumes}')
