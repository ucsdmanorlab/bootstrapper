...
class AddAffErrors(BatchFilter):
    ...
    def __init__(
        ...
        self.context = Coordinate([
            max(abs(offset[dim]) for offset in self.neighborhood)
            for dim in range(3)
        ])
        self.skip = False
    ...
    def prepare(self, request):
        deps = BatchRequest()
        if self.seg_affs in request:

            dims = len(request[self.seg_affs].roi.get_shape())

            # increase segmentation ROI to fit neighborhood
            context_roi = request[self.seg_affs].roi.grow(
                self.context * self.voxel_size, self.context * self.voxel_size
            )
            ...
            deps[self.pred_affs] = request[self.pred_affs].copy()
    ...
    def process(self, batch, request):
        ...
        seg_affs = seg_to_affgraph(seg_array.data, self.neighborhood).astype(np.float32)
