...
@click.command()
@click.option('--in_array', '-i', type=click.Path(exists=True), required=True, help='Path to input Zarr array.')
@click.option('--out_array', '-o', default=None, help='Path to output array')
def bbox(in_array, out_array):
    ...
    if out_array is None:
        out_array = os.path.join(os.path.dirname(in_array), os.path.basename(in_array) + 'bbox')
