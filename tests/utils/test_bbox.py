...
class TestBbox(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.input_array = zarr.open(
            self.temp_dir + "/input.zarr", mode="w", shape=(10, 10, 10), dtype=np.uint8
        )
    ...
    def test_bbox_function(self):
        for args in self.arg_combos:
            ...
            padding = (
                int(args[args.index("--padding") + 1]) if "--padding" in args else 0
            )

            with click.Context(bbox) as ctx:
                result = ctx.invoke(
                    bbox, in_array=in_array, out_array=out_array, padding=padding
                )
