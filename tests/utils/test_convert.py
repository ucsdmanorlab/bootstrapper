...
class TestConvert(unittest.TestCase):
    ...
    def check_output(self, result, args):
        ...
        voxel_size = (
            args[args.index("-vs") + 1 : args.index("-vs") + 4]
            if "-vs" in args
            else (1, 1, 1)
        )
        voxel_offset = (
            args[args.index("-vo") + 1 : args.index("-vo") + 4]
            if "-vo" in args
            else (0, 0, 0)
        )
        axis_names = (
            args[args.index("-ax") + 1 : args.index("-ax") + 4]
            if "-ax" in args
            else ["z", "y", "x"]
        )
        units = (
            args[args.index("-u") + 1 : args.index("-u") + 4]
            if "-u" in args
            else ["nm", "nm", "nm"]
        )
        ...
        self.assertEqual(
            out_ds.offset, tuple(vo * vs for vo, vs in zip(voxel_offset, voxel_size))
        )
    ...
    def test_convert_function(self):
        for args in self.arg_combos:
            ...
            dtype = args[args.index("-d") + 1] if "-d" in args else "uint8"
            voxel_size = (
                tuple(map(int, args[args.index("-vs") + 1 : args.index("-vs") + 4]))
                if "-vs" in args
                else (1, 1, 1)
            )
            voxel_offset = (
                tuple(map(int, args[args.index("-vo") + 1 : args.index("-vo") + 4]))
                if "-vo" in args
                else (0, 0, 0)
            )
            axis_names = (
                tuple(args[args.index("-ax") + 1 : args.index("-ax") + 4])
                if "-ax" in args
                else ("z", "y", "x")
            )
            units = (
                tuple(args[args.index("-u") + 1 : args.index("-u") + 4])
                if "-u" in args
                else ("nm", "nm", "nm")
            )
            crop = "--crop" in args
