...
class TestScalePyramid(unittest.TestCase):
    def setUp(self):
        ...
        self.input_array = zarr.open(
            os.path.join(self.input_path, self.input_ds_name),
            mode="w",
            shape=(64, 64, 64),
            dtype=np.uint8,
        )
        self.input_array[:] = np.random.randint(
            0, 255, size=(64, 64, 64), dtype=np.uint8
        )
    ...
    def test_scale_pyramid_downscale(self):
        ...
        self.assertEqual(
            result.exit_code, 0, f"Command failed with error: {result.exception}"
        )
        zarr_group = zarr.open(self.input_path, mode="r")
        self.assertIn("data/s1", zarr_group, "Downscaled dataset 'data/s1' not found")
        self.assertEqual(
            zarr_group["data/s1"].shape,
            (32, 32, 32),
            "Incorrect shape for downscaled dataset",
        )
    ...
    def test_scale_pyramid_upscale(self):
        ...
        self.assertEqual(
            zarr_group["data/s0"].shape,
            (128, 128, 128),
            "Incorrect shape for upscaled dataset",
        )
