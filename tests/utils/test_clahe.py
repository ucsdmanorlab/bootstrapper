...
class TestClahe(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.input_array = zarr.open(
            self.temp_dir + "/input.zarr", mode="w", shape=(1, 10, 10), dtype=np.uint8
        )
        self.input_array[:] = np.random.randint(
            0, 256, size=(1, 10, 10), dtype=np.uint8
        )
    ...
    def check_function(self, result, args):
        output_path = (
            str(result).strip().split("Output created at ")[-1].strip().split("\n")[0]
        )
    ...
    def test_clahe_function(self):
        for args in self.arg_combos:
            ...
            block_shape = (
                tuple(map(int, args[args.index("-b") + 1 : args.index("-b") + 4]))
                if "-b" in args
                else (1, 256, 256)
            )
            context = (
                tuple(map(int, args[args.index("-c") + 1 : args.index("-c") + 4]))
                if "-c" in args
                else (0, 128, 128)
            )
